...
class Parser(sly.Parser):
    ...
    @_("LET variable '=' expr",
       ...
       "SIN '(' expr ')'")  
    def command(self, p):
        if len(p) == 1:  # Solo una opción
            return p[0]
        else:  # Más de una opción, identifica y maneja cada caso
            if p[0] == "LET":
                return Let(p.variable, p.expr)
            elif p[0] == "READ":
                return Read(p.varlist)
            elif p[0] == "DATA":
                # Implementa la acción correspondiente para DATA
                pass
            elif p[0] == "PRINT":
                # Implementa la acción correspondiente para PRINT
                pass
            # Continúa con el resto de las opciones de manera similar
